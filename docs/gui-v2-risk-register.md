...
| complete inspection workflow and recharge recovery | P0-P6 | Implemented, automated, manual acceptance | [`gui-v2-sim-e2e-smoke.md`](gui-v2-sim-e2e-smoke.md) drives preparation, activation, inspection, depletion, recharge, leave, resume, Hold, and landing through the GUI runtime API; [`gui-v2-real-profile-acceptance.md`](gui-v2-real-profile-acceptance.md) gates real operation stage by stage. | Automated complete-cycle evidence must pass for the tested revision; each real stage requires its own signed record before progression. |
...
All risks from the GUI v2 spec are classified in this register. No open risk remains without an owner, evidence source, and final gate. Classification does
not substitute for the signed real-profile inspection record; unsigned stages
